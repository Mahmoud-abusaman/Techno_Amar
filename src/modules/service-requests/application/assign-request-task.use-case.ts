...
@Injectable()
export class AssignRequestTaskUseCase {
  constructor(
  ...
  async execute(actorId: bigint, taskId: bigint): Promise<PublicRequestTask> {
    ...
    this.workflow.assertTaskAssignable(task, actor.section_id);

    const now = new Date();
    const updatedTask = await this.taskRepo.update(taskId, {
      status: 'IN_PROGRESS',
      assigned_employee_id: actorId,
      assigned_at: now,
    });

    if (task.request.status === RequestStatus.SUBMITTED) {
    ...
    await this.requestRepo.addActivity({
      ...
      description: `Task "${task.name}" assigned to employee`,
    });
