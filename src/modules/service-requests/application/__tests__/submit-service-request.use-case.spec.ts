...
describe('SubmitServiceRequestUseCase', () => {
  ...
  it('submits request for verified citizen and published free service', async () => {
    ...
    const result = await useCase.execute(1n, 1n);
  ...
  it('throws when citizen is not verified', async () => {
    ...
    await expect(useCase.execute(1n, 1n)).rejects.toThrow(BadRequestException);
  });
  ...
  it('throws when service has no workflow tasks', async () => {
    ...
    await expect(useCase.execute(1n, 1n)).rejects.toThrow(ConflictException);
  });
  ...
  it('throws when service not found', async () => {
    ...
    await expect(useCase.execute(1n, 1n)).rejects.toThrow(NotFoundException);
  });
});
