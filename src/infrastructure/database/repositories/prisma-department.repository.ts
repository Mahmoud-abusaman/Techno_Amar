...
export class PrismaDepartmentRepository implements IDepartmentRepository {
  ...
  findById(id: bigint): Promise<DepartmentEntity | null> {
    return this.prisma.department.findUnique({ where: { id } }) as Promise<DepartmentEntity | null>;
  }

  findByName(name: string): Promise<DepartmentEntity | null> {
    return this.prisma.department.findUnique({ where: { name } }) as Promise<DepartmentEntity | null>;
  }

  update(id: bigint, data: UpdateDepartmentData): Promise<DepartmentEntity> {
    return this.prisma.department.update({ where: { id }, data }) as Promise<DepartmentEntity>;
  }
  ...
  async hasDependents(id: bigint): Promise<boolean> {
    const [sectionCount, userCount] = await Promise.all([
      this.prisma.section.count({ where: { department_id: id } }),
      this.prisma.user.count({ where: { department_id: id } }),
    ]);
    return sectionCount > 0 || userCount > 0;
  }
}
